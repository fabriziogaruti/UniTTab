...
def build_baseline_model(model_type, use_embeddings, hidden_size, sequence_len, num_layers, input_size,
                         field_input_size, vocab_size, equal_parameters_baselines=False, loss_weight=None):
    ...
    if model_type == 'lstm':
        if equal_parameters_baselines:
            model = LSTMModelEqual(use_embeddings, sequence_len, num_layers, hidden_size, input_size, field_input_size,
                                   vocab_size, loss_weight)
        else:
            model = LSTMModel(use_embeddings, sequence_len, num_layers, hidden_size, input_size, field_input_size,
                              vocab_size, loss_weight)
    # elif model_type=='mlp':
...
class LSTMModelEqual(nn.Module):
    ...
    def __init__(self, use_embeddings, sequence_len, num_layers, hidden_size, input_size, field_input_size, vocab_size, loss_weight=None):
        super().__init__()
        ...
        self.lstm = nn.LSTM(input_size=self.hidden_size, num_layers=self.num_layers, hidden_size=self.hidden_size,
                            dropout=0.1,
                            batch_first=True)
